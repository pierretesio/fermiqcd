...
/// Make average loops
// ********************************************************************************************************************************
int make_aver_loops_with_CPU(float *RfullCCfield_t,float *IfullCCfield_t,float *RfullCCfield_x,float *IfullCCfield_x,int *box,int nc,int maxloopsize,float* CCstore_aver_loops)
{
   if(nc!=2){printf("nc is not=2!");return(1);}
///allocate memory to receive local  CCstore_loops
   float* CCstore_loops;
   CCstore_loops=(float*)malloc(box[0]*box[1]*box[2]*box[3]*maxloopsize*maxloopsize*sizeof(float));
...
/// compute local loops
   ...
   {          int it,ix;
       ...
       for(tloop=1;tloop<=maxloopsize;tloop++)
       for(xloop=1;xloop<=maxloopsize;xloop++)
       ...
       for(xloc=0;xloc<box[1];xloc++)
         ...
         adr=tloc+box[0]*(xloc+box[1]*(yloc+box[2]*(zloc+box[3]*((tloop-1)+maxloopsize*(xloop-1)))));
...
///compute average loops
   ...
   for(xloop=1;xloop<=maxloopsize;xloop++)
      ...
      av_adr=(tloop-1)+maxloopsize*(xloop-1);
